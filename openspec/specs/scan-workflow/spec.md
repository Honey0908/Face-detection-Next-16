...
Real-time lunch scanning with a 7-state state machine (IDLE → CAMERA_LOADING → SCANNING → MATCHING → SUCCESS/ALREADY_TAKEN/NOT_REGISTERED/ERROR). Detects employee face, matches against database, prevents duplicate daily scans, records lunch, and provides visual/accessibility feedback.
